...
impl Plugin for ScoreManagerPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, spawn_score_entities).add_systems(
            Update,
            ((update_player_scoring, listen_for_game_over_events)
                .chain()
                .in_set(GameRestartSystemSet::Spawning),),
        );
    }
}
...
fn spawn_score_entities(mut commands: Commands) {
    ...
    commands.spawn((LastGameScore::default(), DoNotDestroyOnRestart));
}
...
fn listen_for_game_over_events(
    ...
    mut last_game_score_query: Query<&mut LastGameScore>,
) {
    if read_no_field_variant!(game_events_listener, GameEvent::GameOver).count() > 0 {
        ...
        let mut last_game_score = single_mut_else_return!(last_game_score_query);
        if game_score.0 > best_score.0 {
            best_score.0 = game_score.0;
        }
        last_game_score.0 = game_score.0;
        game_score.0 = 0;
    }
}
