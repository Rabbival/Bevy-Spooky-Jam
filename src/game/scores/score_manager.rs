    ...
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, spawn_score_entities).add_systems(
            ...
            (
                ...
                listen_for_restart_events.in_set(GameRestartSystemSet::Spawning),
            ),
        );
    }
}
...
fn listen_for_game_over_events(
    ...
    stopwatch_query: Query<&PlayerGameStopwatchUi>,
) {
    if read_no_field_variant!(game_events_listener, GameEvent::GameOver).count() > 0 {
        ...
        let stopwatch = single_else_return!(stopwatch_query);
...
fn listen_for_restart_events(
    ...
    if read_no_field_variant!(game_events_listener, GameEvent::RestartGame).count() > 0 {
        single_mut_else_return!(stopwatch_query).timer.reset();
    }
}
