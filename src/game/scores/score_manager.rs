...
fn update_player_scoring(
    ...
    mut world_championship_leaderboard_scoring_query: Query<&mut WorldChampionshipLeaderboardScoring>,
    mut events_reader: EventReader<UpdatePlayerScoreEvent>,
) {
    let Ok(mut world_championship_leaderboard_scoring) = world_championship_leaderboard_scoring_query.get_single_mut() else {
        return;
    };
